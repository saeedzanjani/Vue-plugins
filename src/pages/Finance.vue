<template>
  <div class="main_plugins">
    <span class="title_plugins">Finance Plugins</span>
...
export default {
  ...
  computed: {
    ...
    getActivePlugins() {
      return this.plugins?.data?.data?.tabdata?.tab2?.active?.map((key) => {
        return this.plugins?.data?.data?.plugins[key]
      })
    },
    getDisabledPlugins() {
      return this.plugins?.data?.data?.tabdata?.tab2?.disabled?.map((key) => {
        return this.plugins?.data?.data?.plugins[key]
      })
    },
    getInactivePlugins() {
      return this.plugins?.data?.data?.tabdata?.tab2?.inactive?.map((key) => {
        return this.plugins?.data?.data?.plugins[key]
      })
    },
  },
